...
const VIDEO_SCRIPT_INPUT_KEY = "contentGeneratorVideoScriptInput";
const VIDEO_SCRIPT_RESULT_KEY = "contentGeneratorVideoScriptResult";
const VIDEO_SCRIPT_MODE_KEY = "contentGeneratorVideoScriptMode";
...
function getSavedInput(): SavedInput {
  ...
  const raw = localStorage.getItem(VIDEO_SCRIPT_INPUT_KEY);
...
export default function VideoScriptGeneratorPage() {
  ...
  useEffect(() => {
    ...
    const savedResult = localStorage.getItem(VIDEO_SCRIPT_RESULT_KEY) || "";
    const nextSelectedDay = getSelectedDay();
    const savedMode = localStorage.getItem(VIDEO_SCRIPT_MODE_KEY);
  ...
  const updateSavedInput = (nextValues: Partial<SavedInput>) => {
    ...
    localStorage.setItem(VIDEO_SCRIPT_INPUT_KEY, JSON.stringify(nextInput));
  };

  const updateGenerationMode = (nextMode: GenerationMode) => {
    setGenerationMode(nextMode);
    localStorage.setItem(VIDEO_SCRIPT_MODE_KEY, nextMode);
    setError("");
  };
  ...
  const runLoadingSequence = () => {
    const timers = [
      ...
      setTimeout(() => {
        setLoadingStep(2);
        setLoadingMessage("กำลังเขียนสคริปต์วิดีโอให้เหมาะกับประเด็นนี้...");
      }, 1200),
      setTimeout(() => {
        setLoadingStep(3);
        setLoadingMessage("กำลังเรียบเรียงให้พูดได้ลื่นและพร้อมใช้งาน...");
      }, 2600),
    ];
  ...
  const handleGenerate = async () => {
    ...
    localStorage.setItem(VIDEO_SCRIPT_INPUT_KEY, JSON.stringify(savedInput));

    const timers = runLoadingSequence();

    try {
      const response = await fetch("/api/content-generator/video-script", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !data.content) {
        throw new Error(data.error || "สร้าง Video Script ไม่สำเร็จ");
      }

      setGeneratedContent(data.content);
      localStorage.setItem(VIDEO_SCRIPT_RESULT_KEY, data.content);
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        <div>
          ...
          <p className="text-lg text-gray-700">
            สร้างสคริปต์วิดีโอจริงจากวันที่เลือกใน 30-Day Content Planner
            หรือสร้างจากโครงที่กรอกเองได้ทันที
          </p>
        </div>
        ...
        <div className="border rounded-xl p-6 bg-white space-y-4">
          <div>
            ...
            <p className="text-sm leading-7 text-gray-700">
              ...
              หรือสร้างสคริปต์วิดีโอแบบอิสระจากโครงใหม่
            </p>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            >
              ...
              <div className="text-sm leading-6 text-gray-700">
                ใช้ข้อมูลจาก 30-Day Content Planner เป็นแกนหลักของสคริปต์นี้
              </div>
            </button>
            ...
            >
              ...
              <div className="text-sm leading-6 text-gray-700">
                กรอกโครงวิดีโอใหม่เพื่อสร้างสคริปต์โดยไม่ยึดตามแผน 30 วัน
              </div>
            </button>
          </div>
        </div>
        ...
        {generationMode === "planner" && selectedDay ? (
          <div className="space-y-3">
            <div className="border rounded-xl p-6 bg-gray-50 space-y-3">
              <h2 className="text-2xl font-semibold">วันที่เลือกจากแผนคอนเทนต์</h2>
        ...
        <div className="border rounded-xl p-5 bg-white space-y-4">
          <div>
            <h2 className="text-xl font-semibold mb-2">Brief สำหรับสร้างสคริปต์</h2>
            <p className="text-sm leading-7 text-gray-700">
              {generationMode === "planner"
                ? "ระบบจะใช้วันที่จาก planner เป็นแกนหลัก และคุณสามารถใส่คำสั่งเพิ่มเพื่อปรับโทนหรือวิธีเล่าได้"
                : "กรอกเฉพาะส่วนที่คุณมีไอเดียได้เลย ระบบจะช่วยเรียบเรียงให้กลายเป็นสคริปต์วิดีโอที่พูดได้ลื่นขึ้น"}
            </p>
          </div>
          ...
          {generationMode === "freestyle" && (
            <div className="space-y-5">
              <p className="text-sm text-gray-600">
                ไม่จำเป็นต้องกรอกครบทุกช่อง ถ้ามีแค่แกนหลัก ระบบจะช่วยขยายให้เป็นสคริปต์ที่พูดต่อได้ง่ายขึ้น
              </p>

              <div>
                <label className="font-medium block mb-1">
                  ประเด็นเปิดวิดีโอ <span className="text-red-500">*</span>
                </label>
                <textarea
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border p-3 rounded-lg"
              rows={3}
              placeholder="เช่น ขอให้น้ำเสียงเป็นกันเอง พูดง่าย ใช้ประโยคสั้น และไม่ขายแรง"
            />
          </div>
        </div>
        ...
        {loading && (
          <div className="border rounded-xl p-6 bg-gray-50">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-xl font-semibold">กำลังสร้างสคริปต์วิดีโอ</h2>
            </div>
            ...
            <div className="space-y-3 text-sm">
              ...
              >
                {loadingStep >= 2 ? "✓" : "○"} เขียนสคริปต์ฉบับเต็ม
              </div>
              <div
                className={`flex items-center gap-2 ${
              ...
              >
                {loadingStep >= 3 ? "✓" : "○"} เรียบเรียงให้พร้อมพูด
              </div>
            </div>
          </div>
        )}
        ...
        {generatedContent && (
          <div className="border rounded-xl p-6 bg-white space-y-4">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-2xl font-semibold">ผลลัพธ์ Video Script</h2>
