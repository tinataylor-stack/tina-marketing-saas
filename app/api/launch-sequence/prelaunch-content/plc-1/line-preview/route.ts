  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "ข้อมูลไม่ครบสำหรับสร้างตัวอย่างข้อความ Line OA ของ Prelaunch 1" },
        { status: 400 }
      );
    }
...
Your job is to create short Thai Line OA preview messages for PLC 1 only.

Important:
- This is for PLC 1 only.
- Create exactly 3 short Line OA preview options.
- Each option should tease the full PLC 1 content before the user clicks to read more.
...
4. The options should hint at the opportunity or problem 1.
5. The options should create curiosity toward the full PLC 1 content.
6. Avoid emojis.
`;

    const userPrompt = `
Create 3 Line OA preview options for PLC 1 from the strategy below.
...
`;
    ...
    const response = await openai.responses.create({
      ...
      text: {
        format: {
          type: "json_schema",
          name: "plc_1_line_preview_options",
          schema: {
            type: "object",
    ...
    console.error("PLC 1 LINE PREVIEW ROUTE ERROR:", error);

    return NextResponse.json(
      { error: "เกิดข้อผิดพลาดในการสร้างตัวอย่างข้อความ Line OA ของ Prelaunch 1" },
      { status: 500 }
    );
  }
}
