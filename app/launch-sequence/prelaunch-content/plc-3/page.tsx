...
export default function Plc3ContentPage() {
  ...
  const runPreviewLoadingSequence = () => {
    const timers = [
      setTimeout(() => {
        ...
        setPreviewLoadingMessage("กำลังอ่าน PLC 3 Strategy...");
      }, 0),
      setTimeout(() => {
  ...
  const regenerateLinePreviewOptions = async () => {
    if (!avatarAnalysis || !structuredAvatar || !foundation || !plc3.headline) {
      setError("ข้อมูลไม่ครบสำหรับสร้างตัวอย่างข้อความ Line OA ของ Prelaunch 3");
      return;
    }
    ...
    try {
      const res = await fetch(
        "/api/launch-sequence/prelaunch-content/plc-3/line-preview",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!res.ok) {
        throw new Error(
          data.error || "สร้างตัวอย่างข้อความ Line OA ของ Prelaunch 3 ไม่สำเร็จ"
        );
      }
                ...
                {previewLoading && (
                  <div className="mt-4 rounded-lg border bg-white p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      >
                        {previewLoadingStep >= 1 ? "✓" : "○"} อ่าน PLC 3 Strategy
                      </div>
                      <div
                        className={`flex items-center gap-2 ${
